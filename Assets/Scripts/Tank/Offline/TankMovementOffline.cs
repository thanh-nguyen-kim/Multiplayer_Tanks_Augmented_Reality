...
public class TankMovementOffline : MonoBehaviour
{
    ...
    void OnEnable()
    {
        this.m_IsAR = MenuManager.Instance.ARMode;
        if (m_IsAR)
        {
            MenuManager.Instance.moveBtnDelegate = Move;
            MenuManager.Instance.shotBtnDelegate = Fire;
        }
    ...
    private void Update()
    {
        if (!m_IsAR)
            ...
            {
                //if (GameManagerOffline.s_Instance.clickOnUI) return;
                ray = Camera.main.ScreenPointToRay(Input.mousePosition);
                Physics.Raycast(ray, out hit);
                m_MovementInput = Vector3.Distance(transform.position, hit.point);
                if (!hit.collider) return;
                if (hit.collider.gameObject.tag == "AI" || hit.collider.gameObject.tag == "Building")
                {
                    touchCount++;
        ...
        else
        {
            ...
            if (hit.collider)
            {
                ...
                if (hit.collider.tag == "AI")
                {
                    ...
                    {
                        return;//if shoot a missile do not shot another shell
                    }
                }
