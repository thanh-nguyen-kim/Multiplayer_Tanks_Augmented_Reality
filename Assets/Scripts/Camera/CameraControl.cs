...
public class CameraControl : MonoBehaviour
{
    ...
    private Vector3 FindAveragePosition()
    {
        ...
        for (int i = 0; i < GameManager.m_Tanks.Count; i++)
        {
            ...
            if (GameManager.m_Tanks[i].instance!=null&&!GameManager.m_Tanks[i].instance.activeSelf)
                continue;

            // Add to the average and increment the number of targets in the average.
            average += GameManager.m_Tanks[i].instance.transform.position;
            numTargets++;
        }
    ...
    private float MaxTargetDistance(Vector3 desiredPosition)
    {
        ...
        for (int i = 0; i < GameManager.m_Tanks.Count; i++)
        {
            ...
            if (!GameManager.m_Tanks[i].instance.activeSelf)
                continue;

            // Find the distance from the camera's desired position to the target.
            float targetDistance = (desiredPosition - GameManager.m_Tanks[i].instance.transform.position).magnitude;
